...
| (신규, 원본에 없음) | 타임(1~7타임) 선택 pill 탭 | `.time-tab`, `.time-tab--selected` |
...
640석 그리드는 **어떤 화면 크기에서도 형태가 고정**된다 — 좌석 버튼 크기(52×40px)도, 구역 배치(줄바꿈 없음)도 바뀌지 않는다. A~H 8개 구역은 전부 같은 층이며 위/아래 두 줄로 배치될 뿐이라, 가로 스와이프/스크롤은 `.seat-map__scroll` 하나에서만 일어나고 두 줄이 항상 함께 움직인다 (줄마다 따로 스크롤되면 A구역과 E구역처럼 같은 열의 구역이 어긋나 보이기 때문). 웹이든 모바일이든 동일한 동작이다.
