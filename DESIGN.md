...
Supabase 왕복도 네트워크 상황에 따라 지연이 생길 수 있다. 배정/이동/취소는 전부 같은 패턴을 쓴다: **서버 응답을 기다리지 않고 화면을 먼저 낙관적으로 바꾸고**, `.toast`로 "처리 중 → 완료/실패" 상태를 알려준다. 서버가 거절하면(중복 배정 등) 낙관적으로 바꿔둔 상태를 그대로 되돌리고 실패 토스트를 띄운다. 구현은 [assets/js/seats.js](assets/js/seats.js)의 `showProcessingToast()` + 각 액션 함수의 `prevSeats` 롤백.
