...
2. **그림자는 쓰지 않는다.** 카드, 버튼, 텍스트, 모달 어디에도 그림자를 넣지 않는다. 계층은 배경색 전환(검정 네비 → 프로스티드 서브내비 → 흰 캔버스)으로만 표현한다. 유일한 예외는 `--shadow-glow`(아래 "좌석 글로우" 참조) — 좌석 찾기 결과와 필터 매칭 좌석에만 쓴다.
...
### 예외: 좌석 글로우
"그림자는 쓰지 않는다" 원칙의 유일한 예외, 두 곳에서만 쓴다.
- `.seat--glow` — 좌석 찾기로 찾은 학생의 좌석을 펄스 애니메이션으로 몇 초간 반짝이게 함. 3초 후 자동으로 사라진다.
- `.seat--filter-match` — 학년/반 필터가 켜졌을 때 매칭되는 좌석에 정적(애니메이션 없음) 글로우. 다수의 좌석에 동시에 켜질 수 있어서 펄스 애니메이션은 쓰지 않는다(성능/산만함).

둘 다 `--shadow-glow` 토큰을 공유한다.
...
| (신규, 원본에 없음) | 학년/반 필터 드롭다운 — 검색창 왼쪽, 선택 시 구역 배경이 어두워지고(`--color-surface-tile-1` 재사용) 매칭 좌석만 또렷하게 남는다. "전체" 선택이 곧 exit | `.filter-select`, `.seat-map--filtering`, `.seat--filter-match` |
| (신규, 원본에 없음) | 자리 이동 모드 하단 안내 바(영화관 예매의 좌석 선택 안내) — 이동할 학생의 현재 좌석은 파란 외곽선 | `.move-banner`, `.seat--move-source` |
| (신규, 원본에 없음) | 배정/이동/취소 진행 상태 토스트 — 처리 중/완료/실패 텍스트를 화면 하단에 잠깐 띄운다 | `.toast` (+ `.toast--processing/--success/--error`) |
...
- **스크롤 위치는 리셋될 수 없는 구조**: 15초 폴링/좌석 배정/이동/취소마다 스크롤을 "복원"하는 방식은 스무스 스크롤 애니메이션 도중이거나 브라우저 타이밍에 따라 복원값이 씹히는 문제가 재발했다. 그래서 `renderSeatMap`은 DOM을 최초 1회만 만들고, 이후 호출은 `applySeatState()`로 각 좌석 버튼의 클래스/내용만 제자리에서 갱신한다 — 스크롤 컨테이너 자체를 건드리지 않으므로 스크롤 위치가 애초에 흔들릴 여지가 없다. `container._seatMapBuilt` 플래그로 최초 1회 여부를 판단한다.
- 그래도 `fitToWidth()`는 리사이즈(모바일 브라우저 주소창 접힘 등)로 단독 호출되며 `transform`을 다시 계산하는 동안 스크롤 가능 범위가 잠깐 바뀌므로, 자기 자신의 scrollLeft를 캡처했다가 복원한다. `.seat-map__scroll`에 `scroll-behavior: smooth`가 걸려있어서 이 복원은 `scrollLeft` 대입 전후로 인라인 `style.scrollBehavior = "auto"`를 잠깐 덮어써서 애니메이션 없이 즉시 적용한다.
...
Apps Script 왕복은 보통 1~3초, 가끔 더 걸린다 (Google 플랫폼 자체의 실행 오버헤드라 프론트엔드에서 줄일 수 있는 부분이 아니다). 배정/이동/취소는 전부 같은 패턴을 쓴다: **서버 응답을 기다리지 않고 화면을 먼저 낙관적으로 바꾸고**, `.toast`로 "처리 중 → 완료/실패" 상태를 알려준다. 서버가 거절하면(중복 배정 등) 낙관적으로 바꿔둔 상태를 그대로 되돌리고 실패 토스트를 띄운다. 구현은 [assets/js/seats.js](assets/js/seats.js)의 `showProcessingToast()` + 각 액션 함수의 `prevSeats` 롤백.

## 미해결 / 추가 정의 필요

- 다크모드 대응 (원본 Apple 분석에도 다크모드 카드 스펙은 없음 — 필요 시 별도 설계).
- 폼 검증/에러 상태 스타일 (원본에 미문서화) — 확인이 필요한 액션(체크인 취소)은 `confirm()`, 결과 메시지는 `.toast`, 그 외 나머지 오류는 여전히 `alert()`.
