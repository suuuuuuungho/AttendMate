...
640석 그리드는 **어떤 화면 크기에서도 형태가 고정**된다 — 좌석 버튼 크기(52×40px)도, 구역 배치(줄바꿈 없음)도 바뀌지 않는다. 화면에 다 안 들어오면 층(`.seat-map__floor`)마다 독립적으로 가로 스와이프/스크롤해서 안 보이는 구역을 본다. 웹이든 모바일이든 동일한 동작이다.
